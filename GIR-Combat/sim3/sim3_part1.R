...
samples1 <- cbind(rnorm(n=npeople_type, mean=xmus[comp1],sd=xsds[comp1]),
                  rnorm(n=npeople_type, mean=ymus[comp1],sd=ysds[comp1]))#产生2000个细胞 1000*2
...
A2 <- A2 + matrix(rep(rnorm(nindex), each=npeople_type), ncol=nindex) # gene-specific batch effect (genes are columns)指标(基因)特异性批效(指标(基因)为列)
...
samples3 <- cbind(rnorm(n=npeople_batch3, mean=xmus[comp3],sd=xsds[comp3]),
                  rnorm(n=npeople_batch3, mean=ymus[comp3],sd=ysds[comp3]))
...
A3 <- A3 + matrix(rep(rnorm(nindex), each=npeople_batch3), ncol=nindex) # gene-specific batch effect (genes are columns)指标(基因)特异性批效(指标(基因)为列)
...
data_raw <- cbind(B1_raw,B2_raw,B3_raw)# 100(基因)*300(细胞)

data <- cbind(B1,B2,B3)# 100(基因)*300(细胞)
