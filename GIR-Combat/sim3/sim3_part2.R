...
restrict = NULL # 长度等于...。中对象数量的列表，列表的每个条目对应一个批次，并指定计算校正时要使用的单元格
k=20
prop.k = NULL
sigma=1 # 一个数值标量，指定用于计算每个单元的校正向量的高斯平滑内核的带宽。
var.adj=TRUE; subset.row = NULL; correct.all = FALSE;
merge.order = NULL; auto.merge = FALSE;assay.type = "logcounts";

cos.norm.in = TRUE # 示在计算单元格之间的距离之前是否应对输入数据执行余弦归一化。
cos.norm.out = TRUE
svd.dim = 0L # 一个整数标量，指定用于汇总每个批次内的生物子结构的维数。
...
.prepare_input_data <- function (batches, cos.norm.in, cos.norm.out, subset.row, correct.all) 
{
  ...
  if (!is.null(subset.row)) {
    ...
    if (identical(subset.row, seq_len(nrow(batches[[1]])))) {
      ...
      in.batches <- lapply(in.batches, "[", i = subset.row, drop = FALSE)
      if (correct.all) {
        same.set <- FALSE
      }
      else {
...
in.batches <- prep.out$In # 2个100(基因)*150(细胞)
out.batches <- prep.out$Out
subset.row <- prep.out$Subset
same.set <- prep.out$Same
in.batches <- lapply(in.batches, t) # 2个150(细胞)*100(基因)
if (!same.set) {
  out.batches <- lapply(out.batches, t)
}
if (!auto.merge) {# 选择合并顺序。
