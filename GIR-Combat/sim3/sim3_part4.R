...
######################
png(
  filename = "experment3.png", # 文件名称 # 原来的测试，每张图片的名字还是中文
  width = 2700,           # 宽
  height = 2250,          # 高
  units = "px",          # 单位
  bg = "white",          # 背景颜色
...
data_raw_t <- t(data_raw)
sponge.lm.data_raw <- lm(data_raw_t[ ,9] ~  comp_cluster + batch.id)
summary(sponge.lm.data_raw)

uncorrected_t <- t(uncorrected) 
sponge.lm.before <- lm(uncorrected_t[ ,9] ~  comp_cluster + batch.id)
...
combat_edata_addmod_real_noref_t<- t(combat_edata_addmod_real_noref) 
...
last_combat_edata_addmod_t<- t(last_combat_edata_addmod)
...
last_combat_edata_addmod_real_t<- t(last_combat_edata_addmod_real) 
...
combat_edata_addmod_noref_t<- t(combat_edata_addmod_noref) 
...
png(
  filename = "experment31.png", # 文件名称 # 原来的测试，每张图片的名字还是中文
  width = 1875,           # 宽
  height = 1000,          # 高
  units = "px",          # 单位
  bg = "white",          # 背景颜色
...
asw_score <- function(Matrix0,Batch_type,comp_cluster){
  ...
  for( i in 1:20){
    ...
    sih.Cell1 =(sih.Cell1+1)/2 # 其实有个公式，带进去min_val = -1 ，max_val = 1
    sih.batch1 = (sih.batch1+1)/2 # 其实有个公式，带进去min_val = -1 ，max_val = 1
