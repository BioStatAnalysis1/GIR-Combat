...
for (mdx in seq_len(nmerges)) {
                                ...
                                k, prop.k = NULL, ...) 
  {
    ...
    findMutualNN <- function (data1, data2, k1, k2 = k1, BNINDEX1 = NULL, BNINDEX2 = NULL) 
    {
      ...
      W21 <- queryAnnoy(data2, query=data1, k=k2, get.distance=FALSE) # W21 <- do.call(queryKNN, args)# 查询批次1中的关于批次2的近邻对,W21  150*20矩阵
      ...
      out <- find_mutual_nns(W21$index, W12$index)# batch1和batch2共找到1610个相互最近邻对
  ...
  .compute_correction_vectors <- function (data1, data2, mnn1, mnn2, tdata2, sigma) 
  {
    ...
    cell.vect <- dusmooth_gaussian_kernel(assay(averaged, withDimnames = FALSE), 
                                        averaged$ID - 1L, tdata2, sigma) # cell.vect u ⃗(x)的转置 
  
    t(cell.vect)
  }
  
  correction.in <- .compute_correction_vectors(left.data, 
                                               right.data, s1, s2, trans.right, sigma) # correction.in u ⃗(x)
