...
png( 
  filename = "realdata1.png", # 文件名称 # 原来的测试，每张图片的名字还是中文
  width = 2700,           # 宽
  height = 2667,          # 高
  units = "px",          # 单位
  bg = "white",          # 背景颜色
...
###################################################################改
uncorrected <- data # 2 1045(指标) *704(人)
Xcom <- combat_data # 3 1045(指标) *704(人)
MNN_correct_t <- Xmnn # 4 1045(指标) *704(人)
RefMnn_t <- last_combat_edata # 6 1045(指标) *704(人)
...
sponge.lm.before <- lm(uncorrected_t[ ,9] ~  people_kind + batch.id)# 第9个指标
summary(sponge.lm.before)

Xcom_t<- t(Xcom) # 3 # 704(人)*1045(指标)
sponge.lm.combat <- lm(Xcom_t[ ,9] ~ people_kind +  batch.id)
summary(sponge.lm.combat)



MNN_correct # 4 # 300(人)*50(主成分)
sponge.lm.MNN <- lm(MNN_correct[ ,9] ~ people_kind +  batch.id)
summary(sponge.lm.MNN)



last_combat_edata_addmod_t<- t(last_combat_edata_addmod) # 7 # 704(人)*1045(指标)
sponge.lm.last_combat_edata_addmod <- lm(last_combat_edata_addmod_t[ ,9] ~ people_kind +  batch.id)
...
sponge.lm.last_combat_edata_addmod_real <- lm(last_combat_edata_addmod_real_t[ ,9] ~ people_kind +  batch.id)
...
sponge.variance <- c(
  # as.vector(sponge.varmat.data_raw), # 1
  as.vector(sponge.varmat.before), # 2
  as.vector(sponge.varmat.combat), # 3
  # as.vector(sponge.varmat.combat_edata_addmod_real_noref), # 9
  as.vector(sponge.varmat.mnn), # 4
  # as.vector(sponge.varmat.RefMnn),  # 6
  # as.vector(sponge.varmat.last_combat_edata_addmod) # 7
  as.vector(sponge.varmat.last_combat_edata_addmod_real) # 8
  # as.vector(sponge.varPart.combat_edata_addmod_noref)
)# 10

sponge.variance <- cbind(variance = sponge.variance,
                         Type = rep(c('Cancer','Batch'), each = ncol(uncorrected_t)),
                         method = rep(c(
                           # 'No Batch Effect',
                           'Raw',
                           'ComBat',
...
png( 
  filename = "realdata11.png", # 文件名称 # 原来的测试，每张图片的名字还是中文
  width = 2000,           # 宽
  height = 1000,          # 高
  units = "px",          # 单位
  bg = "white",          # 背景颜色
...
kBET_score <- function(Matrix0, Batch_type ){
  # Matrix0 <- t(uncorrected)#  300(样本)* 100(指标)
  # Batch_type <- batch.id
  ...
  avg_score <- sum(rejection$stats$kBET.observed)/n_repeat # 另外一个evalution_utils,但是和他的结果不一样
  avg_score
}
...
asw_score <- function(Matrix0,Batch_type,comp_cluster){
  # Matrix0 <- dat.pca_uncorrected$ind$coord # 57(人)*20(主成分)
  # Batch_type <- batch.id
